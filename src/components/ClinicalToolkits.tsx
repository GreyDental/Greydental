    ...
    <section className="w-[100vw] relative left-1/2 -translate-x-1/2 bg-[#F4F7F8] py-12 sm:py-16 md:py-20 lg:py-24 overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col gap-8 sm:gap-10 md:gap-12 w-full min-w-0">
        ...
        <div className="flex flex-col items-start text-left gap-3 sm:gap-4 w-full min-w-0">
          ...
          <p className="text-[#777779] font-regular_18pt text-[14px] sm:text-[15px] md:text-[16px] leading-relaxed max-w-2xl">
        ...
        <div className="flex flex-col lg:flex-row gap-5 sm:gap-6 w-full min-w-0">
          {columns.map((col) => (
            <div
              key={col.heading}
              className="flex flex-col flex-1 min-w-0 w-full bg-[#F4F7F8] border border-[#BCBCBC] rounded-[16px] sm:rounded-[20px] p-5 sm:p-6 md:p-7"
            >
              <h3 className="text-[18px] sm:text-[20px] md:text-[22px] font-semi_bold_24pt text-[#3A738D] leading-snug tracking-tight text-center mb-5 sm:mb-6">
                {col.heading}
              </h3>
              ...
              <div className="flex flex-col gap-3 sm:gap-3.5 w-full min-w-0">
                {col.items.map((item) => (
                  ...
                  >
                    <div className="flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 rounded-[8px] bg-[#2F5F75] flex-shrink-0">
                      <Image
                        src={col.icon}
                        alt=""
                        width={16}
                    ...
                    <div className="flex flex-col flex-1 min-w-0">
                      <p className="text-[#3A738D] font-semi_bold_24pt text-[13px] sm:text-[14px] md:text-[15px] leading-snug truncate">
                        {item.title}
                      </p>
                      <p className="text-[#777779] font-regular_18pt text-[12px] sm:text-[13px] mt-0.5">
                        {item.meta}
                      </p>
                    </div>
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-[#5ECAA066] flex-shrink-0 hover:bg-[#5ECAA0] transition-colors"
                    >
                      <svg
                        width="14"
                        height="14"
