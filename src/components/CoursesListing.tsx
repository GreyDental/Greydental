...
export default function CoursesListing() {
  ...
  const showWorkshops = activeTab === "Workshops" || activeTab === "All";
  const showWebinars = activeTab === "Webinars" || activeTab === "All";
  const tintedSections = activeTab === "All";

  return (
    <>
      <section
        className={`w-full bg-white pt-12 sm:pt-16 md:pt-20 ${
          showCourseFilters ? "pb-12 sm:pb-16 md:pb-20" : "pb-6 sm:pb-8"
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col">
          {/* Tabs */}
          <div className="flex flex-row flex-wrap items-center gap-x-5 gap-y-2 sm:gap-x-8 border-b border-[#E5E7EB] w-full">
            {tabs.map((tab) => (
              <button
                key={tab}
                type="button"
          ...
          {showCourseFilters && (
            <>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center gap-3 sm:gap-4 bg-[#F2FAFF] border border-[#F3F4F6] rounded-[12px] p-3 sm:p-4 mb-8 sm:mb-10 w-full">
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 lg:items-center w-full lg:w-auto">
              ...
              <div className="flex flex-col md:flex-row md:flex-wrap gap-5 sm:gap-6 w-full">
                {filtered.map((course) => (
                  <article
                    key={course.id}
      ...
      {showWorkshops && <WorkshopsSection tinted={tintedSections} />}
      {showWebinars && <WebinarsSection tinted={tintedSections} />}
    </>
  );
}
