...
export default function CommunityMembership() {
  return (
    <section
      id="membership"
      className="w-[100vw] relative left-1/2 -translate-x-1/2 bg-[#F4F7F8] py-14 sm:py-16 md:py-20 lg:py-24 overflow-x-hidden flex flex-col"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 w-full min-w-0 flex flex-col">
        ...
        <div className="flex flex-col items-center text-center gap-4 sm:gap-5 mb-10 sm:mb-12 md:mb-14">
          <div className="inline-flex items-center px-4 py-1.5 rounded-full border border-[#777779] bg-transparent">
        ...
        <div className="flex flex-col sm:flex-row sm:flex-wrap xl:flex-nowrap gap-4 sm:gap-5 md:gap-6 w-full min-w-0 items-stretch">
          {plans.map((plan) => (
            <article
              key={plan.name}
              className="group flex flex-col min-w-0 w-full sm:w-[calc(50%-0.625rem)] xl:flex-1 xl:w-auto h-auto bg-white border-2 border-[#E0E0E0] rounded-[16px] sm:rounded-[20px] p-6 sm:p-7 transition-all duration-300 hover:bg-[#2F5F75] hover:border-[#3A738D] hover:shadow-[0_5px_12px_rgba(0,0,0,0.10),0_22px_22px_rgba(0,0,0,0.09),0_40px_32px_rgba(0,0,0,0.05)]"
            >
              <h3 className="text-[#2F5F75] group-hover:text-white font-semi_bold_24pt text-[16px] sm:text-[17px] md:text-[18px] leading-snug mb-3 sm:mb-4 transition-colors">
                {plan.name}
              </h3>

              <div className="flex items-baseline gap-1 mb-4 sm:mb-5">
                <span className="text-[#3A738D] group-hover:text-white font-semi_bold_24pt text-[32px] sm:text-[36px] md:text-[40px] leading-none tracking-tight transition-colors">
                  {plan.price}
                </span>
                {plan.period && (
                  <span className="text-[#777779] group-hover:text-white/55 font-regular_18pt text-[13px] sm:text-[14px] transition-colors">
                    {plan.period}
                  </span>
                )}
              </div>
              ...
              <ul className="flex flex-col gap-3 sm:gap-3.5 flex-1 mb-6 sm:mb-8">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-2.5 min-w-0">
                    ...
                    />
                    <span className="text-[#3A738D] group-hover:text-white/90 font-regular_18pt text-[13px] sm:text-[14px] leading-snug transition-colors">
                      {feature}
                    </span>
                  </li>
                ))}
              ...
              <Link
                href={plan.href}
                className="mt-auto inline-flex items-center justify-center w-full bg-white border border-[#3A738D] text-[#3A738D] group-hover:bg-[#5ECAA0] group-hover:border-[#5ECAA0] group-hover:text-black px-5 py-3 sm:py-3.5 rounded-[10px] font-inter-medium_18pt text-[14px] sm:text-[15px] transition-colors"
              >
                {plan.cta}
              </Link>
            </article>
