...
export default function WhyPartner() {
  return (
    <section
      id="why-partner"
      className="w-[100vw] relative left-1/2 -translate-x-1/2 bg-[#F4F7F8] py-14 sm:py-16 md:py-20 lg:py-24 overflow-hidden flex flex-col"
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 w-full min-w-0 flex flex-col">
        ...
        <div className="flex flex-col sm:flex-row sm:flex-wrap gap-4 sm:gap-5 w-full min-w-0">
          {reasons.map((reason) => (
            <article
              key={reason.title}
              className="flex flex-col min-w-0 w-full sm:w-[calc(50%-0.625rem)] bg-white border border-[#E5E7EB] border-l-[4px] border-l-[#5ECAA0] rounded-[12px] sm:rounded-[14px] px-5 sm:px-6 md:px-7 py-5 sm:py-6 shadow-[0_4px_16px_rgba(58,115,141,0.06)]"
            >
              <h3 className="text-[#2F5F75] font-semi_bold_24pt text-[16px] sm:text-[17px] md:text-[18px] leading-snug tracking-tight mb-2 sm:mb-2.5">
                {reason.title}
              </h3>
